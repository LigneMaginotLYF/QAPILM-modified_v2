...
## Mean estimate and confidence bands

### Mean estimate

The **mean estimate** is the pointwise mean of the decoded C (or U) field
over *all* Monte Carlo realizations within a run:

```
mean_C(x, z) = (1/N_mc) * Σᵢ  C_i(x, z)
```

where `C_i` is the C field decoded from the i-th MC weight vector `coef_i`.
**Weight vectors are never averaged**; only the decoded fields are.

For U: each MC realization's C estimate is used to run the forward solver,
producing a per-realization U field.  The mean U is the average over these
N_mc forward-solver outputs.

### Confidence band

The confidence band is **mean ± k·std** at each grid / time point:

```
lower(x) = mean_C(x) - k * std_C(x)
upper(x) = mean_C(x) + k * std_C(x)
```

The multiplier `k` is set by the `BAND_K` parameter in the batch plotting
scripts (default `1.0`):

| `BAND_K` | Interpretation (Gaussian ensemble) |
|----------|-------------------------------------|
| `1.0`    | ≈ 68 % interval (mean ± 1σ) |
| `2.0`    | ≈ 95 % interval (mean ± 2σ) |
| `3.0`    | ≈ 99.7 % interval (mean ± 3σ) |

For a **single-run** result (`N_mc == 1`) the confidence band collapses to a
single curve (std = 0).
...
bands (mean ± k·std across the MC ensemble) for an epsilon-threshold batch
...
| Variable | Purpose |
|----------|---------|
| `BAND_K` | Confidence-band multiplier (default `1.0` → ±1σ) |
